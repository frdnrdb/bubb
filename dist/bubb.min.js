...
var bubb = function(config, callback)  {
  ...
  */
  ...
  var bubbs = Array.from( document.querySelectorAll('[data-bubb]:not(.bubb)') );
...
bubb.update = function(key, contentOrConfig)  {return update(key, contentOrConfig)};
bubb.add = bubb.refresh = function()  {return addOrRemove.apply(this$0, arguments)};
bubb.remove = function()  {return addOrRemove.apply(this$0, arguments)};

var buildElement = function(_bubbParent ) {

  var key = _bubbParent.dataset.bubb.trim(),
      ...
      var bindMenu = typeof bubb.callback === 'function' || typeof bubb.config._.callback === 'function';
      ...
      var html = props.reduce( buildConent.bind(this$0, key), '' );
...
var configureElement = function(_bubbParent, _bubbLe, html)  {

    _bubbLe = _bubbLe || document.createElement('aside');

    var config = _bubbParent.bubb.config,
        update = !html;
...
var bindElement = function(_bubbParent ) {
  ...
  function callbackHandler(e) {
      ...
      var hover = _bubbParent.bubb.config.hoverCallback,
          bubbvalue = hover ? this.dataset.bubb : e.target.dataset.bubbValue || e.target.parentNode.dataset.bubbValue;

      if (!bubbvalue) return;

      var thiscallback = (typeof this.bubb.config.callback === 'function' && this.bubb.config.callback) || bubb.callback,
...
var update = function(key, contentOrConfig)  {

      if (typeof key !== 'string' || !contentOrConfig) return;

      var updateConfig = typeof contentOrConfig === 'object',
      ...
      var _bubbParent = parent || item.parentNode.parentNode; // .bubb < aside < div (item)

      if (updateConfig) {

        var bindDefault = typeof contentOrConfig.callback === 'boolean' && !_bubbParent.bubb.bind,
        ...
        var _bubbLe = parent ? parent.children[0] : item.parentNode;
...
var addOrRemove = function()  {
  ...
  if (!value && bubb.config[menu.key][menu.val]) {
    ...
    var _bubbLe = document.querySelector((("[data-bubb=\"" + (menu.key)) + "\"]")).children[0];
    Array.from(_bubbLe.children).forEach( function(child ) {
...
var updateMainConfig = function(key, contentOrConfig, updateConfig, _bubbParent)  {

  var typeMenu = _bubbParent.bubb.type === 'menu',
...
var isMobile = (typeof window.orientation !== "undefined") || ~navigator.userAgent.indexOf('IEMobile') ? true : false;
